...
export async function* streamOpenAiCompletions(
	...
	while (true) {
		...
		for (const line of lines) {
			for (const event of parseStreamLine(line)) yield event;
		}
	}

	for (const event of parseStreamLine(buffer)) yield event;
}
...
function applyThinkingOptions(
	...
	if (format === "deepseek") {
		...
		if (effort && supportsReasoningEffort) body.reasoning_effort = effort;
		return;
	}
	if (format === "together") {
...
function normalizeThinkingLevel(
	...
	return !level || level === "off" ? undefined : level;
}
